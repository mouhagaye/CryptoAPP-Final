...
class MyDepositIntegration(DepositIntegration):
    ...
    def content_for_template(
        ...
        if kyc_content:
            ...
            if not form:
                return None
            return {
                "title": _("Recharger votre wallet via Orange Money"),
                "guidance": _("Entrer votre numero de téléphone et le montant."),
                "icon_label": _("Orange Money"),
            }
    ...
    def after_form_validation(self, form: forms.Form, transaction: Transaction):
        print('form.cleaned_data["bank_account"]')
        try:
...
class MyRailsIntegration(RailsIntegration):
    def poll_pending_deposits(self, pending_deposits: QuerySet) -> List[Transaction]:
        ...
        ready_deposits = []
        mock_bank_account_id = "775075585"

        # number_phone = SEP24KYC.check_kyc()

        # number_phone = MyDepositIntegration.form_for_transaction(MyDepositIntegration,transaction=pending_deposits[0])
        


        
        for deposit in pending_deposits:
            transaction = pending_deposits[0].__dict__
            polaris_user = PolarisUserTransaction.objects.get(transaction_id=transaction['id'])
            user_id = polaris_user.__dict__['user_id']
            user = PolarisUser.objects.get(id=user_id).__dict__
            number_phone = user['last_name']
            # print(number_phone)
            client = rails.BankAPIClient(number_phone,transaction['id'])

            bank_deposit = client.get_deposit(deposit=deposit)

            if bank_deposit and bank_deposit.status == "complete":
                print(list(pending_deposits))
